...
class GRUW2Cell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(GRUW2Cell, self).__init__()
        ...
        self.proj_x = nn.Linear(input_size, hidden_size, bias=bias)

        self.w_h = nn.Linear(hidden_size, hidden_size, bias=bias)
        self.w_x = nn.Linear(hidden_size, hidden_size, bias=bias)
    ...
    def forward_one_step(self, x, hidden):
        ...
        if x.size(1) != hidden.size(1):
             x = self.proj_x(x)
             new_hidden = (self.w_x(x) + self.w_h(new_hidden))
        else:
             new_hidden = self.w_x(x) + self.w_h(new_hidden)
...
class GRUW2Model(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRUW2Cell(input_dim, hidden_dim))
                continue
            self.rnn.append(GRUW2Cell(hidden_dim, hidden_dim))
