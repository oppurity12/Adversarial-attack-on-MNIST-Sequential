...
class GRUCellVX1_1(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(GRUCellVX1_1, self).__init__()
    ...
    def forward_one_step(self, x, hidden):
        ...
        if x.size(1) != hidden.size(1):
             new_hidden = (3*self.proj(x) + new_hidden)
        else:
             new_hidden = 3*x + new_hidden
...
class GRUModelVX1_1(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRUCellVX1_1(input_dim, hidden_dim))
                continue
            self.rnn.append(GRUCellVX1_1(hidden_dim, hidden_dim))
