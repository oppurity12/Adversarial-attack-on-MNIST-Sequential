...
class GRUWCell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(GRUWCell, self).__init__()
    ...
    def forward_one_step(self, x, hidden):
        ...
        if x.size(1) != hidden.size(1):
             new_hidden = (self.proj_x(x) + self.w_h(new_hidden))
        else:
             new_hidden = x + self.w_h(new_hidden)
...
class GRUWModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRUWCell(input_dim, hidden_dim))
                continue
            self.rnn.append(GRUWCell(hidden_dim, hidden_dim))
