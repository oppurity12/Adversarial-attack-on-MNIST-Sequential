...
class GRULearningAlphaCell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(GRULearningAlphaCell, self).__init__()
...
class GRULearningAlphaModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        super(GRULearningAlphaModel, self).__init__()
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRULearningAlphaCell(input_dim, hidden_dim))
                continue
            self.rnn.append(GRULearningAlphaCell(hidden_dim, hidden_dim))
