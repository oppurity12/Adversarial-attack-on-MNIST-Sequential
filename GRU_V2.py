...
class GRUCellV2(nn.Module):
    ...
    def forward_one_step(self, x, hidden):
        ...
        if x.size(1) != hidden.size(1):
             new_hidden = (self.proj(x) + 2*new_hidden)
        else:
             new_hidden = x + 2*new_hidden
...
class GRUModelV2(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRUCellV2(input_dim, hidden_dim))
                continue
            self.rnn.append(GRUCellV2(hidden_dim, hidden_dim))
