...
class GRUCellBeta(nn.Module):
    def __init__(self, input_size, hidden_size, beta, bias=True):
        super(GRUCellBeta, self).__init__()
    ...
    def forward_one_step(self, x, hidden):
        ...
        if x.size(1) != hidden.size(1):
             new_hidden = (self.beta * self.proj(x) + new_hidden)
        else:
             new_hidden = self.beta * x + new_hidden
...
class GRUModelBeta(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, alpha, batch_first=False):
        super(GRUModelBeta, self).__init__()
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(GRUCellBeta(input_dim, hidden_dim, alpha))
                continue
            self.rnn.append(GRUCellBeta(hidden_dim, hidden_dim, alpha))
