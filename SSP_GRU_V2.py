...
class SSRGRU2Cell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(SSRGRU2Cell, self).__init__()
    ...
    def forward(self, x, hidden):
        self.alpha_2_1 = torch.clamp(self.alpha_2_1, 0, 1)
        self.beta_1_0 = torch.clamp(torch.abs(self.beta_1_0), 0, 1)
...
class SSRGRUModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, batch_first=False):
        ...
        for i in range(layer_dim):
            if i == 0:
                self.rnn.append(SSRGRU2Cell(input_dim, hidden_dim))
                continue
            self.rnn.append(SSRGRU2Cell(hidden_dim, hidden_dim))
